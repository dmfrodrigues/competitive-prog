...
vector< set<NUMT> > solve(){
    ...
    while(true){ ++counter;
        ...
        NUMT c, v;{
            set<STR2ORD>::iterator it;
            while(true){
                it = Process.begin();
                c = it->c; v = it->v;
                auto itfind = ToChange.find(v);
                if(itfind == ToChange.end()){
                    break;
                }else{
                    for(NUMT c = 0; c < C; ++c){
        ...
        NUMT bestc, bestv; bool changed = false; {
            ...
            for(NUMT c = 0; c < C; ++c){
                for(NUMT v = 0; v < V; ++v){
                    if(S[v] > Avail[c]) continue; ///Se o video _v_ nao cabe na cache _c_, avançar
                    if(ToChange.find({c,v}) != ToChange.end()){ ToChange.erase(ToChange.find({c,v}));
                        double& g = Gain[c][v];
                        g = 0;
                        for(const auto& e:Prov[c]){
                            NUMT d = getCurrDiff(e.first, v, ans); ///Se _e_ pede o video _v_, quanto é que ganha atualmente
        ...
        */
        ...
        Avail[bestc] -= S[bestv];
        for(const auto& p:Prov[bestc])
            ToChange.insert({p.first, bestv});
    }
    return ans;
}
